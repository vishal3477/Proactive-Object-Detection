...
if __name__ == '__main__':
  ...
  optimizer_3 = torch.optim.Adam(encoder_model.parameters(), lr=0.0001)
  ...
  load_name = os.path.join(args.model_path, './faster_rcnn_1_11_4000.pth')
  load_name_signal = os.path.join(args.model_path, './state__1_11_4000.pickle')
  ...
  checkpoint_signal = torch.load(load_name_signal)
  checkpoint = torch.load(load_name)
    
  fasterRCNN.load_state_dict(checkpoint['model'])
  encoder_model.load_state_dict(checkpoint_signal['state_dict_encoder'])
  ...
  flag1=0
  all_gt_boxes=100*torch.ones((5000,100))
  for i in range(num_images):
      ...
      if flag1==0:
        ...
        all_scores=cls_prob.data.detach()
        
        flag1=1
      
      else:
        all_scores=torch.cat([all_scores,cls_prob.data.detach()], dim=0)
        
      all_gt_boxes[i,0:int(gt_boxes[0,:,4].shape[0])]=gt_boxes[0,:,4].detach()
